...
function sproto.dump(obj)
	for k,v in pairs(obj.__tcache) do 
		print("?", k, v)
	end
end

function sproto.parse(ptext)
	local parser = require "app.tool.sprotoparser"
	local pbin = parser.parse(ptext)
	return sproto.new(pbin)
end

function sproto.parsefile(path)
	local file = io.open(path, "r")
    if file then
		local content = file:read("*a")
        io.close(file)
        return sproto.new(content)
	else 
		print("找不到文件:" .. path)
	end
end
